...
public class Team {
    ...
    /**
     * Empty-roster team for typed JSON hydrate. Season stats stay 0 until TEAM_SEASON is applied.
     * Career W-L on the team are the pre-season totals from the save document.
     */
    public static Team createHydrateShell(
            League league,
            String conference,
            String name,
            String abbr,
            ...
            int totalBowlLosses,
            boolean showPopups,
            TeamStreak yearStartWinStreak,
            boolean teamTVDeal,
            boolean confTVDeal,
            OffensivePhilosophy offPhilosophy,
            DefensiveSystem defSystem,
            boolean programProfileUpdatedThisOffseason,
            QbPressurePolicy qbPressurePolicy,
            String evenYearHomeOpp) {
        Team t = new Team(league, true);
        t.conference = conference;
        t.name = name;
        t.abbr = abbr;
        t.programProfile = profile;
        t.totalWins = careerWins;
        t.totalLosses = careerLosses;
        t.totalCCs = totalCCs;
        t.totalNCs = totalNCs;
        t.rivalries = rivalries != null ? new ArrayList<>(rivalries) : new ArrayList<>();
        t.totalNCLosses = totalNCLosses;
        t.totalCCLosses = totalCCLosses;
        t.totalBowls = totalBowls;
        t.totalBowlLosses = totalBowlLosses;
        t.showPopups = showPopups;
        TeamStreak streak = yearStartWinStreak != null
                ? yearStartWinStreak
                : new TeamStreak(league.getYear(), league.getYear(), 0, abbr);
        t.winStreak = streak;
        t.yearStartWinStreak = new TeamStreak(
                streak.getStartYear(), streak.getEndYear(), streak.getStreakLength(), streak.getTeam());
        t.teamTVDeal = teamTVDeal;
        t.confTVDeal = confTVDeal;
        t.offPhilosophy = offPhilosophy != null ? offPhilosophy : OffensivePhilosophy.MULTIPLE;
        t.defSystem = defSystem != null ? defSystem : DefensiveSystem.BASE_4_3;
        t.programProfileUpdatedThisOffseason = programProfileUpdatedThisOffseason;
    ...
    /** Private empty shell used by {@link #createHydrateShell}. */
    private Team(League league, boolean hydrateShell) {
        if (!hydrateShell) {
            throw new IllegalArgumentException("hydrateShell must be true");
        }
        this.league = league;
        userControlled = false;
        showPopups = true;
    ...
    /** Group roster players into FR/SO/JR/SR/Grad lists (typed hydrate). */
    public void regroupClassStanding() {
        teamFRs.clear();
        teamSOs.clear();
        teamJRs.clear();
        teamSRs.clear();
    ...
    /**
     * Year-1 NIL purse cost must fit the current purse; future years encumber NIL only.
     * Scholarships have no purse cost (checked separately via canAwardScholarship).
     */
    public boolean canAffordContract(int yearOneNilPurse, int futureAnnualNil, int years) {
        if (yearOneNilPurse > recruitMoney) return false;
    ...
    public boolean canAffordContract(RosterStatus status, int nilAmount, int years) {
        int yearOne = nilPurseCost(status, nilAmount);
    ...
    /** Annual NIL purse dollars for an offer (scholarships are free). */
    public int nilPurseCost(RosterStatus status, int nilAmount) {
        return NilMoney.nilPurseCost(status, nilAmount, programProfile);
    }

    public OfferResourceCost offerResources(RosterStatus status, int nilAmount) {
        return NilMoney.offerResources(status, nilAmount);
    }
    ...
    public boolean spendOffer(RosterStatus status, int nilAmount, int years) {
        int cost = nilPurseCost(status, nilAmount);
    ...
    public String budgetHeader() {
        return "Budget: " + NilMoney.format(recruitMoney)
                ...
                + " · Scholarships: " + getScholarshipCount() + "/" + NilMoney.SCHOLARSHIP_CAP
    ...
    public static class TeamStatLine {
        public final String value;
        public final String label;
        public final String rank;

        public TeamStatLine(String value, String label, String rank) {
            this.value = value;
            this.label = label;
            this.rank = rank;
        }
    ...
    /**
     * Structured team stats and rankings for UI consumers.
     */
    public List<TeamStatLine> getTeamStatLines() {
        List<TeamStatLine> lines = new ArrayList<>();
        ...
        for (Conference c : league.conferences) {
            if (c.confName.equals(conference)) {
                ...
                lines.add(new TeamStatLine(
                        getConfWins()+"-"+getConfLosses(),
                        "Conf W-L",
                        getRankStr(confRank) + "%"));
            }
        }
        ...
        lines.add(new TeamStatLine(
                String.valueOf(teamPollScore),
                "AP Votes",
                getRankStr(rankTeamPollScore) + "%"));
        lines.add(new TeamStatLine(
                String.valueOf(teamStrengthOfWins),
                "SOS",
                getRankStr(rankTeamStrengthOfWins) + "%"));
        lines.add(new TeamStatLine(
                String.valueOf(teamPoints/numGames()),
                "Points",
                getRankStr(rankTeamPoints) + "%"));
        lines.add(new TeamStatLine(
                String.valueOf(teamOppPoints/numGames()),
                "Opp Points",
                getRankStr(rankTeamOppPoints) + "%"));
        lines.add(new TeamStatLine(
                String.valueOf(teamYards/numGames()),
                "Yards",
                getRankStr(rankTeamYards) + "%"));
        lines.add(new TeamStatLine(
                String.valueOf(teamOppYards/numGames()),
                "Opp Yards",
                getRankStr(rankTeamOppYards) + "%"));
        lines.add(new TeamStatLine(
                String.valueOf(teamPassYards/numGames()),
                "Pass Yards",
                getRankStr(rankTeamPassYards) + "%"));
        lines.add(new TeamStatLine(
                String.valueOf(teamRushYards/numGames()),
                "Rush Yards",
                getRankStr(rankTeamRushYards) + "%"));
        lines.add(new TeamStatLine(
                String.valueOf(teamOppPassYards/numGames()),
                "Opp Pass YPG",
                getRankStr(rankTeamOppPassYards) + "%"));
        lines.add(new TeamStatLine(
                String.valueOf(teamOppRushYards/numGames()),
                "Opp Rush YPG",
                getRankStr(rankTeamOppRushYards) + "%"));
        lines.add(new TeamStatLine(
                teamTODiff > 0 ? "+" + teamTODiff : String.valueOf(teamTODiff),
                "TO Diff",
                getRankStr(rankTeamTODiff) + "%"));
        lines.add(new TeamStatLine(
                String.valueOf(teamOffTalent),
                "Off Talent",
                getRankStr(rankTeamOffTalent) + "%"));
        lines.add(new TeamStatLine(
                String.valueOf(teamDefTalent),
                "Def Talent",
                getRankStr(rankTeamDefTalent) + "%"));
        lines.add(new TeamStatLine(
                String.valueOf(programProfile.programPower),
                "Program Power",
                getRankStr(rankTeamProgramPower) + "%"));
        lines.add(new TeamStatLine(String.valueOf(programProfile.tradition), "Tradition", "—%"));
        lines.add(new TeamStatLine(String.valueOf(programProfile.fanbase), "Fanbase", "—%"));
        lines.add(new TeamStatLine(String.valueOf(programProfile.donors), "Donors", "—%"));
        lines.add(new TeamStatLine(String.valueOf(programProfile.footprint), "Recruiting Footprint", "—%"));
        lines.add(new TeamStatLine(String.valueOf(programProfile.pipeline), "NFL Pipeline", "—%"));
        lines.add(new TeamStatLine(String.valueOf(programProfile.momentum), "Momentum", "—%"));
        lines.add(new TeamStatLine(
                NilMoney.format(NilMoney.yearlyBudget(programProfile)),
                "Purse",
                "—%"));
        lines.add(new TeamStatLine(
                String.valueOf(getRecruitingClassRat()),
                "Recruit Class",
                getRankStr(rankTeamRecruitClass) + "%"));
    ...
    public String getTeamStatsStrCSV() {
        ...
        for (TeamStatLine line : getTeamStatLines()) {
            ts0.append(line.value).append(",");
            ts0.append(line.label).append(",");
            ts0.append(line.rank).append("\n");
        }
        return ts0.toString();
    }
