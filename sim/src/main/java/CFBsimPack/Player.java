...
public class Player {
     ...
     * Caller pays the year-1 NIL purse installment; remaining years are future encumbrance.
     * NIL requires a scholarship: SCHOLARSHIP + positive NIL upgrades to SCHOLARSHIP_PLUS_NIL;
     * PWO cannot carry NIL (amount is zeroed).
     */
    public void applyOffer(RosterStatus status, int nilAmount, int years) {
        RosterStatus st = status != null ? status : RosterStatus.SCHOLARSHIP;
        int nil = Math.max(0, nilAmount);
        if (st == RosterStatus.PWO) {
            nil = 0;
        } else if (nil > 0 && st == RosterStatus.SCHOLARSHIP) {
            st = RosterStatus.SCHOLARSHIP_PLUS_NIL;
        }
        this.rosterStatus = st;
        this.nilDealAmount = (st == RosterStatus.SCHOLARSHIP_PLUS_NIL) ? nil : 0;
    ...
    public int annualDealCash(ProgramProfile profile) {
        return NilMoney.nilPurseCost(rosterStatus, nilDealAmount, profile);
    }
