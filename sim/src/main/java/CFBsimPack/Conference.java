...
public class Conference {
    ...
    public void restoreCcg(Game game) {
        ccg = game;
    }
    ...
    public ArrayList<Player> getAllConfPlayers() {
        if (allConfPlayers.isEmpty()) {
            ...
            addTopAllConference(qbs, 1);
            addTopAllConference(rbs, 2);
            addTopAllConference(fbs, 1); // optional when pool empty
            addTopAllConference(wrs, 3);
            addTopAllConference(tes, 1); // optional when pool empty
            addTopAllConference(ols, 5);
            addTopAllConference(ks, 1);
            addTopAllConference(ss, 1);
            addTopAllConference(cbs, 3);
            addTopAllConference(edges, 2);
            addTopAllConference(dls, 3);
            addTopAllConference(lbs, 3);
        }
    ...
    private void addTopAllConference(ArrayList<Player> pool, int count) {
        int n = Math.min(count, pool.size());
        for (int i = 0; i < n; i++) {
            Player p = pool.get(i);
            allConfPlayers.add(p);
            p.wonAllConference = true;
        }
    }
