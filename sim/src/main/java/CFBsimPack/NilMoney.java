...
public final class NilMoney {
    ...
    /**
     * Resources spent by an offer: scholarship slots (cap) and annual NIL purse cash.
     * Scholarships and PWOs cost $0; only SCHOLARSHIP_PLUS_NIL spends the purse.
     */
    public static OfferResourceCost offerResources(RosterStatus status, int nilAmount) {
        if (status == null || status == RosterStatus.PWO) {
            return new OfferResourceCost(0, 0);
        }
        if (status == RosterStatus.SCHOLARSHIP_PLUS_NIL) {
            return new OfferResourceCost(1, Math.max(0, nilAmount));
        }
        return new OfferResourceCost(1, 0);
    }

    /** NIL purse cost of an offer (dollars). Scholarships and PWOs are free. */
    public static int nilPurseCost(RosterStatus status, int nilAmount, ProgramProfile profile) {
        return nilPurseCost(status, nilAmount);
    }

    public static int nilPurseCost(RosterStatus status, int nilAmount, int ignoredScore) {
        return nilPurseCost(status, nilAmount);
    }

    public static int nilPurseCost(RosterStatus status, int nilAmount) {
        return offerResources(status, nilAmount).annualNilCash;
    }
    ...
    public static int buyoutCost(Player p, ProgramProfile profile) {
        ...
        int annual = nilPurseCost(p.rosterStatus, p.nilDealAmount, profile);
